...
def spectral_loss(output, output2, target, target2, wavenum_init,wavenum_init_ydir,lambda_reg,ocean_grid):
    '''
        Spectral regularizer + MSE for multiple timesteps
    ...
    loss = ((1-lambda_reg)*loss1 + 0.25*(lambda_reg)*loss2 + 0.25*(lambda_reg)*loss2_ydir\
         + 0.25*(lambda_reg)*loss2_next_time+0.25*(lambda_reg)*loss2_ydir_next_time)
...
def one_epoch(batch_size, optimizer, net, lambda_reg, lead, trainN, wavenum_init, wavenum_init_ydir, delta_t, timesteps = False):
    ...
    for k in range(1993, 2020): # loop through years of data
        ...
        psi_train_input_Tr_torch = psi_train_input_Tr_torch.to(DEVICE)
        psi_train_label_Tr_torch = psi_train_label_Tr_torch.to(DEVICE)
        psi_train_label2_Tr_torch = psi_train_label2_Tr_torch.to(DEVICE)
        psi_train_input_Tr_torch_norm_level1 = ((psi_train_input_Tr_torch[:,0,None,:,:]-M_test_level1.to(DEVICE)))/STD_test_level1.to(DEVICE)
        psi_train_label_Tr_torch_norm_level1 = ((psi_train_label_Tr_torch[:,0,None,:,:]-M_test_level1.to(DEVICE)))/STD_test_level1.to(DEVICE)
        psi_train_label2_Tr_torch_norm_level1 = ((psi_train_label2_Tr_torch[:,0,None,:,:]-M_test_level1.to(DEVICE)))/STD_test_level1.to(DEVICE)
        ...
        for step in range(0,trainN-batch_size,batch_size): # loop through batches
            ...
            label_batch2 = psi_train_label2_Tr_torch_norm_level1[idxs,:,:,:]
            ...
            output2 = PECstep(net, output.to(DEVICE), delta_t)
            
            label_batch2 = label_batch2.to(DEVICE).permute(0,2,3,1)
            loss = spectral_loss(
                output.to(DEVICE),
                output2.to(DEVICE),
                label_batch.to(DEVICE), 
                label_batch2.to(DEVICE), 
...
def objective(trial):
    ...
    net.load_state_dict(torch.load('./Models/FNO_single_PECstep.pt'))
